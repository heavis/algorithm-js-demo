    ...
    for (let j = 0; j < s.length; j++) {
        if (matchMap.get(s[j]) !== undefined) {
            ...
            if (mapVal[0].length >= mapVal[1]) {
                const deleted = mapVal[0].shift()
                // console.log(deleted)
                console.log('deted:' + deleted)

                indexArr.splice(indexArr.indexOf(deleted), 1)
            ...
            if (count === t.length) {
                ...
                //     })
                // })
                const min = indexArr[0], max = indexArr[indexArr.length - 1]
